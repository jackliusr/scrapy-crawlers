...
from crawlers.items import PrincipalItem

class PrincipalsSpider(BaseSpider):
    name = "principals"
	...
	yield Request(url ="http://sis.moe.gov.sg/Pages/SchoolDetails/GeneralInformation.aspx?schoolCode={0}".format(code), callback=self.parse_principal)
	
    def parse_principal(self, response):
      sel = Selector(response)
      item = PrincipalItem()      
      item["schoolCode"] = response.url[-4:]
      item['name'] = sel.xpath("//div[@id='principalNames']/text()").extract()
      yield item 
